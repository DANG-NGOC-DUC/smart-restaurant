...
function Users() {
  const [mainTab, setMainTab] = useState("staff"); // "staff" | "guest"
  ...
  const staffData = useAdminUsers("staff");
  const guestData = useAdminUsers("guest");
  const active = mainTab === "guest" ? guestData : staffData;
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-cream-100/50">
      ...
      {/* Main Tabs */}
      <div className="flex items-center gap-1 bg-white rounded-xl p-1 shadow-card border border-sea-100/60 w-fit">
        <button
          onClick={() => {
            setMainTab("staff");
            setSelectedRole("Tat ca");
            setSearchQuery("");
          }}
